module Tools
  class ListSleeps < BaseTool
    ...
    input_schema(
      properties: {
        limit: {type: "number", description: "Limit on the number of sleeps returned (max 25, default 10)"},
        start: {type: "string", format: "date-time", description: "Return sleeps that occurred after or during (inclusive) this time. If not specified, the response will not filter sleeps by a minimum time."},
        end: {type: "string", format: "date-time", description: "Return sleeps that intersect this time or ended before (exclusive) this time. If not specified, end will be set to now."},
